...
class TokenUtilsTest {
  ...
  void parseUserTokenFromCookies_positive() {
    ...
    var authResponse = new AuthnClient.LoginResponse(accessExp.toString(), refreshExp.toString());
    var accessToken = "acc";
  ...
  @ParameterizedTest
  @CsvSource({",1970-01-01T00:00:00Z", "1970-01-01T00:00:00Z,"})
  void parseUserTokenFromCookies_negative_invalidExpiration(String accessExp, String refreshExp) {
    var authResponse = new AuthnClient.LoginResponse(accessExp, refreshExp);
    var accessToken = "acc";
    var refreshToken = "ref";
    var cookies = List.of(new DefaultCookie(FOLIO_ACCESS_TOKEN, accessToken).toString(),
      new DefaultCookie(FOLIO_REFRESH_TOKEN, refreshToken).toString());
  ...
  @ParameterizedTest
  @CsvSource({"folioAccessToken,folioRefreshToken", "folioRefreshToken, folioAccessToken"})
