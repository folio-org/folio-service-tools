...
class MessageBatchProcessorTest {
  ...
  private void verifyFailedMessage(Pair pair, int expectedValue) {
    assertThat(pair.value()).isEqualTo(expectedValue);
  ...
  private Consumer<List<Integer>> attemptThrowingConsumer(List<Integer> list, AtomicInteger cnt, int max) {
    return values -> {
