...
public class TokenUtils {
  ...
  public static UserToken parseUserTokenFromCookies(List<String> cookieHeaders, AuthnClient.LoginResponse loginResponse) {
    var cookies = cookieHeaders.stream()
      .map(String::trim)
    ...
    var refreshToken = getTokenFromCookies(FOLIO_REFRESH_TOKEN, cookies);

    return UserToken.builder()
      .accessToken(accessToken)
      .accessTokenExpiration(parseExpiration(loginResponse.accessTokenExpiration()))
      .refreshToken(refreshToken)
      .refreshTokenExpiration(parseExpiration(loginResponse.refreshTokenExpiration()))
      .build();
  }
